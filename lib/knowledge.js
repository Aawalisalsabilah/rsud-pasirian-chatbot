...
async function getKamarText(q) {
  ...
  return `\nDATA KETERSEDIAAN KAMAR RAWAT INAP (DIPERBARUI TERAKHIR: ${tanggalUpdateText}) — INI SATU-SATUNYA SUMBER DATA YANG BOLEH DIPAKAI UNTUK MENJAWAB PERTANYAAN SOAL KAMAR/BED:\n${daftarText}\n\nData di atas diambil LANGSUNG dari sistem rawat inap RSUD Pasirian dan SUDAH AKURAT — jangan ragukan angkanya. Jangan mengarang jumlah bed atau ketersediaan kamar yang tidak ada di daftar ini. Jika ada teks/referensi lain di prompt ini (misalnya dari knowledge base umum) yang menyebut angka ketersediaan kamar/bed yang BERBEDA dari daftar di atas, ABAIKAN sepenuhnya teks tersebut — anggap sudah usang, dan gunakan HANYA angka dari daftar di atas. Jika semua kamar dalam satu kelas berstatus PENUH, sampaikan apa adanya dan sarankan user menghubungi bagian informasi RSUD Pasirian atau IGD untuk kondisi darurat.`;
}
...
5. Jika suatu blok data referensi di bawah ini memuat bagian berjudul "INSTRUKSI JAWABAN CHATBOT", kamu WAJIB memakai kalimat jawaban yang tertulis di situ APA ADANYA (boleh dirapikan formatnya, tapi isi & maksudnya tidak boleh diubah), dan DILARANG KERAS menambahkan langkah, prosedur, istilah, atau detail apa pun yang TIDAK tertulis secara eksplisit di blok data tersebut (misalnya nomor antrean digital, kode QR, mesin anjungan mandiri, atau istilah lain yang terdengar masuk akal tapi tidak ada sumbernya) — meskipun terdengar wajar berdasarkan alur pendaftaran RS pada umumnya. Jika informasi yang ditanyakan user tidak tercakup oleh instruksi tersebut, arahkan user menghubungi kontak yang disebutkan di data, JANGAN mengarang tambahan.
6. DILARANG KERAS mengarang, menebak, atau "mengingat" nomor telepon, nomor WhatsApp, atau kontak apa pun (termasuk untuk IGD, pendaftaran, poli, atau layanan lainnya) yang TIDAK tertulis secara eksplisit dan literal dalam blok data referensi di bawah ini. Jika user menanyakan kontak/nomor telepon suatu layanan dan nomornya TIDAK ada di data, JANGAN menuliskan nomor apa pun — sampaikan bahwa nomor tersebut belum tersedia di data dan arahkan ke nomor Info Layanan resmi (0334) 5761044 yang memang tercantum di data, atau ke kontak lain yang benar-benar tertulis di blok data. Jika data yang relevan dengan pertanyaan user TIDAK ditemukan sama sekali di bawah ini (misalnya soal biaya, harga, atau detail suatu layanan), JANGAN menjawab dengan mengarang — sampaikan apa adanya bahwa informasi tersebut belum tersedia dan arahkan ke Info Layanan (0334) 5761044.`
...
export async function buildSystemPrompt(currentQuestion = '', selectedPoli = null, contextQuery = '') {
  ...
  const kamarTriggered = KEYWORDS_KAMAR.some((k) => q.includes(k));

  let relevantText = '';
  if (!isPendaftaranAmbiguous) {
    const relevantChunks = await searchRelevantKnowledge(effectiveContext);
    const filteredChunks = kamarTriggered
      ? relevantChunks.filter((chunk) => !KEYWORDS_KAMAR.some((k) => (chunk.judul || '').toLowerCase().includes(k)))
      : relevantChunks;
    relevantText = filteredChunks
