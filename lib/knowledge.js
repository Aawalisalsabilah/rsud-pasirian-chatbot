...
async function getBlobData() {
  ...
  // PENTING: file yang di-upload dengan access:'private' tidak bisa
  // diambil pakai fetch() biasa — wajib disertai header Authorization
  // pakai token yang sama dengan yang dipakai untuk list().
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `Gagal mengambil knowledge.json dari Blob. Status: ${response.status} ${response.statusText}. Body: ${bodyText.slice(0, 200)}`
    );
  }
