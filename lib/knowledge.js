...
// Parser generik untuk teks hari kayak "Senin-Kamis", "Selasa & Kamis",
// "Senin, Rabu, Jumat", atau nama hari tunggal "Jumat" — tanpa perlu
// didaftar manual satu-satu, jadi kombinasi baru otomatis kebaca.
...
// Field `hari` dan `jam` di Supabase bisa berisi beberapa baris (dipisah \n)
// untuk satu dokter dengan slot berbeda, misal:
//   hari: "Senin-Kamis\nJumat"
//   jam:  "08.30-12.00\n12.30-14.30"
// Fungsi ini memecahnya jadi array slot {hari, jam} yang berpasangan per baris.
...
// Deteksi nama hari eksplisit yang disebut user (mis. "Kamis", "hari Senin").
// Tanpa ini, pertanyaan yang nyebut nama hari (bukan kata relatif seperti
// "besok"/"lusa") bakal salah fallback ke "hari ini".
function detectNamaHariEksplisit(q) {
  return NAMA_HARI_LOWER.find((h) => q.includes(h)) || null;
}

// Coba resolve kata waktu (relatif atau nama hari) dari satu string.
// Return null kalau gak nemu apa-apa, biar caller bisa coba string lain.
function tryResolveWaktu(q, today) {
  if (q.includes('lusa')) return { offsetDays: 2, label: 'lusa' };
  if (q.includes('besok')) return { offsetDays: 1, label: 'besok' };
  if (q.includes('minggu depan')) return { offsetDays: 7, label: 'minggu depan' };

  const hariEksplisit = detectNamaHariEksplisit(q);
  if (hariEksplisit) {
    const todayIdx = today.getDay();
    const targetIdx = NAMA_HARI_LOWER.indexOf(hariEksplisit);
    let diff = targetIdx - todayIdx;
    if (diff <= 0) diff += 7; // ambil kejadian terdekat ke depan kalau hari yg sama/udah lewat minggu ini
    return { offsetDays: diff, label: hariEksplisit.charAt(0).toUpperCase() + hariEksplisit.slice(1) };
  }

  return null;
}

// `currentQuestion` = pesan user PALING BARU, dicoba resolve duluan.
// `fallbackContext` = gabungan pesan-pesan sebelumnya, dipakai HANYA kalau
// pesan terbaru sama sekali gak nyebut kata waktu apapun. Urutan prioritas
// ini penting: kalau digabung rata tanpa prioritas, kata waktu dari pesan
// LAMA bisa keambil duluan dan nutupin kata waktu di pesan yang baru
// diketik user (mis. "Rabu" dari histori vs "Kamis" dari pesan sekarang).
...
// `currentQuestion` = pesan user paling baru (dipakai buat deteksi hari,
// harus paling prioritas). `contextQuery` = gabungan beberapa pesan user
// terakhir termasuk currentQuestion (dipakai buat deteksi topik/poli, biar
// follow-up question yang gak nyebut ulang nama poli tetap nyambung).
export async function buildSystemPrompt(currentQuestion = '', selectedPoli = null, contextQuery = '') {
  ...
  const parts = [ATURAN_KEAMANAN, pendaftaranClarificationText, baseInfo, infoHariIni, infoLibur, relevantText, jadwalPoliText].filter(Boolean);
